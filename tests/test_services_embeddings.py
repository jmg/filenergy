...
def test_is_configured_true_in_test():
    assert embeddings.is_configured()


def test_search_returns_empty_when_unconfigured(db, workspace, monkeypatch):
    monkeypatch.setattr(embeddings, "is_configured", lambda: False)
    assert embeddings.search(workspace, "hi", k=3) == []


def test_search_returns_empty_when_no_chunks(db, workspace):
    assert embeddings.search(workspace, "hi", k=3) == []


def test_search_returns_empty_for_none_workspace(db):
    assert embeddings.search(None, "hi", k=3) == []


def test_search_ranks_by_cosine(db, user, workspace):
    f = File(
        user_id=user.id, workspace_id=workspace.id,
        name="a.txt", path="/tmp/a.txt", url="h",
    )
    db.session.add(f)
    db.session.commit()
    db.session.add_all([
...
def test_search_filters_by_workspace(db, user, workspace):
    ...
    f1 = File(user_id=user.id, workspace_id=workspace.id, name="mine.txt", path="/tmp/mine", url="h1")
    f2 = File(user_id=other.id, workspace_id=other_ws.id, name="theirs.txt", path="/tmp/theirs", url="h2")
    ...
    results = embeddings.search(workspace, "x", k=10)
